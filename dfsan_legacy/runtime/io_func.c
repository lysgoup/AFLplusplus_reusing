...
#include "dtaint_legacy_compat.h" /* stands in for ffds.h + len_label.h -- see that file */

static int granularity = 1; // byte level

/* Angora's own __angora_track_fini/_rs (a Rust FFI logger-flush hook) is
   dropped here -- our dtaint_logger.c registers its own
   __attribute__((destructor)) fini function (dtaint_logger_fini) that fires
   automatically at process exit, so nothing needs to call it explicitly. */
